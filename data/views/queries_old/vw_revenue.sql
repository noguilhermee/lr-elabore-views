...
SELECT
    ...
    SUM(CASE WHEN type = 'MILK_SOLD' THEN amount_total ELSE 0 END) AS milk_sold_revenue,
    SUM(CASE WHEN type = 'MILK_SOLD' THEN COALESCE((payload::jsonb ->> 'quantity')::numeric, 0) ELSE 0 END) AS milk_volume_sold,
    MAX(CASE WHEN type = 'MILK_SOLD' THEN (payload::jsonb ->> 'unit_price')::numeric END) AS milk_unit_price,
    ...
    MAX(CASE WHEN type = 'MILK_DERIVATIVES' THEN (payload::jsonb ->> 'unit_price')::numeric END) AS unit_price_derivative,
    MAX(CASE WHEN type = 'MILK_DERIVATIVES' THEN (payload::jsonb ->> 'quantity')::numeric END) AS milk_volume_derivatives,
    
    SUM(CASE WHEN type = 'MILK_DERIVATIVES' THEN amount_total ELSE 0 END) AS milk_derivatives_revenue,
    ...
    SUM(CASE WHEN type = 'CONCENTRATED_SOLD' THEN amount_total ELSE 0 END) AS concentrated_sold,
