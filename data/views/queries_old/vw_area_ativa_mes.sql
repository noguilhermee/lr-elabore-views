 ...
 SELECT amv.id_property,
    ...
    sum(
        CASE
            WHEN amv.is_app_reserva THEN amv.hectares_total
            ELSE 0::double precision
        END) AS hectares_app_reserva,
    sum(
        CASE
            WHEN NOT amv.is_app_reserva THEN amv.hectares_total
            ELSE 0::double precision
        END) AS hectares_atividade,
    sum(amv.hectares_total) AS hectares_area_total,
    sum(amv.land_value_month * amv.hectares_owned) / NULLIF(sum(
        CASE
            WHEN amv.land_value_month IS NOT NULL THEN amv.hectares_owned
            ELSE 0::double precision
        END), 0::double precision) AS land_value_propria,
    sum(
        CASE
            WHEN amv.is_app_reserva THEN amv.land_value_month * amv.hectares_total
            ELSE 0::double precision
        END) / NULLIF(sum(
        CASE
            WHEN amv.is_app_reserva AND amv.land_value_month IS NOT NULL THEN amv.hectares_total
            ELSE 0::double precision
        END), 0::double precision) AS land_value_app_reserva,
    sum(
        CASE
            WHEN NOT amv.is_app_reserva THEN amv.land_value_month * amv.hectares_total
            ELSE 0::double precision
        END) / NULLIF(sum(
        CASE
            WHEN NOT amv.is_app_reserva AND amv.land_value_month IS NOT NULL THEN amv.hectares_total
            ELSE 0::double precision
        END), 0::double precision) AS land_value_atividade,
    sum(amv.land_value_month * amv.hectares_total) / NULLIF(sum(
        CASE
            WHEN amv.land_value_month IS NOT NULL THEN amv.hectares_total
            ELSE 0::double precision
        END), 0::double precision) AS land_value_area_total
